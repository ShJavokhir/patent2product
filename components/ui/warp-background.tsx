...
const Beam = ({
  ...
  if (!beamProps) {
    // Return a placeholder div with the same structure but no animation during SSR
    return (
      <motion.div
        style={
          {
            ...
            "--width": `${width}`,
            "--aspect-ratio": "1",
            "--background": color || "linear-gradient(hsl(0 80% 60%), transparent)",
      ...
      style={
        {
          "--x": `${x}`,
          "--width": `${width}`,
          "--aspect-ratio": `${beamProps.ar}`,
          "--background": color || `linear-gradient(hsl(${beamProps.hue} 80% 60%), transparent)`,
        } as React.CSSProperties
      }
      className={`absolute left-[var(--x)] top-0 [aspect-ratio:1/var(--aspect-ratio)] [background:var(--background)] [width:var(--width)]`}
      initial={{ y: "100cqmax", x: "-50%" }}
